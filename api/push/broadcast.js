...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(500).json({
        ok: false,
        error: "No se pudieron obtener las suscripciones"
      });
    }
    ...
    if (!subscriptions.length) {
      return res.status(404).json({
        ok: false,
        error: "No hay clientes suscriptos"
      });
    }
    ...
    for (
      ...
      try {
         ...
         * 404 / 410 significa que podemos eliminarla.
         */
        ...
        if (
          ...
          try {
            ...
            // No interrumpimos el envío por un error de limpieza.
          }
        }
      }
    }
